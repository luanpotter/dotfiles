...
_audit_pacman() {
	...
	while true; do
		...
		case "$action" in
		Import*)
			_audit_pacman_import "${unmanaged[@]}"
			return 0
			;;
		Uninstall*)
			_audit_pacman_uninstall "${unmanaged[@]}"
			return 0
			;;
		List*)
			printf '%s\n' "${unmanaged[@]}" | gum pager
...
_audit_apt() {
	...
	# -- collect all declared apt packages (from modules + bootstrap)
	local -A declared=()
	...
	if [[ ${#unmanaged_all[@]} -eq 0 ]]; then
		log_ok "audit: all ${#installed[@]} manually installed packages are tracked"
		return 0
	fi

	# -- leaf filtering: keep only packages no other unmanaged package depends on
	local -A depended_on=()
	for pkg in "${unmanaged_all[@]}"; do
		local dep
	...
	log_warn "audit: ${#unmanaged[@]} unmanaged leaf package(s) out of ${#installed[@]} manually installed (${#unmanaged_all[@]} total unmanaged)"
	...
	while true; do
		...
		case "$action" in
		Import*)
			_audit_apt_import "${unmanaged[@]}"
			return 0
			;;
		Uninstall*)
			_audit_apt_uninstall "${unmanaged[@]}"
			return 0
			;;
		List*)
			printf '%s\n' "${unmanaged[@]}" | gum pager
...
_audit_apt_import() {
	...
	if [[ "$target_pick" == "[+] Create new YAML under os/" ]]; then
		local rel
		if ! rel=$(gum input --placeholder "relative path under os/, e.g. debian/misc.yaml"); then
			return 0
		fi
		rel="${rel/#\//}"
		rel="${rel#"os/"}"
		[[ -z "$rel" ]] && return 0
		if [[ "$rel" == *..* ]] || [[ "$rel" == /* ]]; then
			log_error "audit: path must be relative with no .. (e.g. debian/misc.yaml)"
			return 1
		fi
		case "$rel" in
		*.yaml | *.yml) ;;
	...
	if [[ "$grouping" == Separate* ]]; then
		local pkg
		for pkg in "${selected[@]}"; do
			_audit_append_module_block "$target_file" "$pkg" "apt:$pkg"
		done
		log_ok "audit: added ${#selected[@]} separate module(s) to $target_file"
		return 0
	fi
...
_audit_apt_uninstall() {
	...
	log_warn "audit: will uninstall ${selected[*]}"
	if gum confirm "Uninstall ${#selected[@]} package(s)?"; then
		sudo apt-get remove --purge -y "${selected[@]}"
		log_ok "audit: uninstalled ${#selected[@]} package(s)"
	else
		log_info "audit: cancelled"
	fi
